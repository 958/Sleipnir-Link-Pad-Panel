...
static HRESULT APIPRIVATE OLE_IDropTarget_Internal_SendMessage_Drop(LPDROPTARGET pThis, UINT uNotify, LPDATAOBJECT pdo, DWORD grfKeyState, POINTL *ppt, LPDWORD pdwEffect);
...
BOOL APIPRIVATE OLE_IDropTarget_RegisterDragDrop(HWND hWnd, HWND hWndMsg, UINT uCallbackMessage, UINT *cFormat, int cfcnt)
{
	...
	pdti->m_refCnt = 0;
	pdti->hWnd = hWndMsg;												/* メッセージを受け取るウィンドウ */
...
static HRESULT STDMETHODCALLTYPE OLE_IDropTarget_Drop(LPDROPTARGET pThis, LPDATAOBJECT pdo, DWORD grfKeyState, POINTL pt, LPDWORD pdwEffect)
{
	return OLE_IDropTarget_Internal_SendMessage_Drop(pThis, IDROPTARGET_NOTIFY_DROP, pdo, grfKeyState, &pt, pdwEffect);
}
